...
select has_function(
  'dem_private', 'verify_policy',
  'Function verify_policy should exist'
);

grant all on table dem.test_table to test_role;

create policy select_test on dem.test_table
  for select to test_role using (true);
create policy insert_test on dem.test_table
  for insert to test_role with check (true);
create policy update_test on dem.test_table
  for update to test_role using (true) with check(true);
create policy delete_test on dem.test_table
  for delete to test_role using (true);
create policy all_test on dem.test_table
...
select lives_ok(
  $$
    select dem_private.verify_policy('select', 'select_test', 'test_table', 'test_role');
  $$,
...
select lives_ok(
  $$
    select dem_private.verify_policy('insert', 'insert_test', 'test_table', 'test_role');
  $$,
...
select lives_ok(
  $$
    select dem_private.verify_policy('update', 'update_test', 'test_table', 'test_role');
  $$,
...
select lives_ok(
  $$
    select dem_private.verify_policy('delete', 'delete_test', 'test_table', 'test_role');
  $$,
...
select lives_ok(
  $$
    select dem_private.verify_policy('all', 'all_test', 'test_table', 'test_role');
  $$,
...
select throws_ok(
  $$
    select dem_private.verify_policy('bubbles', 'select_test', 'test_table', 'test_role');
  $$,
  'P0001',
...
select throws_ok(
  $$
    select dem_private.verify_policy('insert', 'select_test', 'test_table', 'test_role');
  $$,
  'P0001',
...
select throws_ok(
  $$
    select dem_private.verify_policy('select', 'bubbles', 'test_table', 'test_role');
  $$,
  'P0001',
...
select throws_ok(
  $$
    select dem_private.verify_policy('select', 'select_test', 'not_a_table', 'test_role');
  $$,
  '42P01',
  'relation "dem.not_a_table" does not exist',
  'Function verify_policy throws if table name does not exist in policy'
);

select throws_ok(
  $$
    select dem_private.verify_policy('select', 'select_test', 'test_table', 'not_a_role');
  $$,
  'P0001',
