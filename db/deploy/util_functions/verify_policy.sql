-- Deploy dem:database_functions/verify_policy to pg
-- requires: schema_dem

begin;

-- Verifies if a policy exists
create or replace function dem_private.verify_policy(
  operation text,
  policy_name text,
...
comment on function dem_private.verify_policy(text, text, text, text, text) is
