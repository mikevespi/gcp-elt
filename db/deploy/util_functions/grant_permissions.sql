-- Deploy dem:util_functions/grant_permissions to pg

begin;

-- Grants permission for all columns in a table
create or replace function dem_private.grant_permissions(
  operation text,
  table_name text,
  role_name text,
  table_schema text default 'dem'
)
returns void
as
$function$
...
comment on function dem_private.grant_permissions(text, text, text, text) is
...
comment on function dem_private.grant_permissions(text, text, text, text[], text) is
