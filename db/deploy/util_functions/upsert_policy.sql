-- Deploy dem:database_functions/upsert_policy to pg
...
-- example: select dem_private.upsert_policy('test_policy', 'test_table', 'update', 'admin', 'true');
-- becomes: create policy test_policy on dem.test_table for update to admin using(true) with check(true);
create or replace function dem_private.upsert_policy(
  policy_name text,
  table_name text,
  operation text,
  role_name text,
  using_check_statement text,
  table_schema_name text default 'dem'
)
returns void
as
$function$
...
-- example: select dem_private.upsert_policy('test_policy', 'test_table', 'update', 'admin', 'using(true)', 'with check(false)');
-- becomes: create policy test_policy on dem.test_table for update to admin using(true) with check(false);
create or replace function dem_private.upsert_policy(
  policy_name text,
  table_name text,
  operation text,
...
comment on function dem_private.upsert_policy(text, text, text, text, text, text) is
  'A generic function for creating or altering row-level security policies';
comment on function dem_private.upsert_policy(text, text, text, text, text, text, text) is
