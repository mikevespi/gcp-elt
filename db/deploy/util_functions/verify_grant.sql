-- Deploy dem:database_functions/verify_grants to pg
-- requires: schema_dem

begin;

-- Verifies permission grants for all columns in a table
create or replace function dem_private.verify_grant(
  operation text,
...
comment on function dem_private.verify_grant(text, text, text, text) is
...
comment on function dem_private.verify_grant(text, text, text, text[], text) is
