-- Deploy dem:util/upsert_timestamp_columns to pg
-- requires: trigger_function/update_timestamps

begin;

create or replace function dem_private.upsert_timestamp_columns(
  table_schema_name text,
  table_name text,
  add_create boolean default true,
  add_update boolean default true,
  add_archive boolean default true,
  user_table_schema_name text default 'dem',
  user_table_name text default 'dem_user'
)
returns void as $$
...
begin
    ...
    if add_archive = true then
      ...
      if not exists (select *
        ...
        trigger_string := concat(
          ...
          ' for each row execute procedure dem_private.archived_records_are_immutable()'
        );
  ...
  if not exists (select *
    ...
    trigger_string := concat(
      ...
      ' for each row execute procedure dem_private.update_timestamps()'
    );
    execute(trigger_string);
  end if;
...
comment on function dem_private.upsert_timestamp_columns(text, text, boolean, boolean, boolean, text, text)
  is $$
